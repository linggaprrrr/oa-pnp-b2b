            ...
            <form action="/selections/" method="get">
                <label class="block">
                    ...
                    <span class="relative mt-1.5 flex">
                        <input name="date"  class="filter-date form-input peer w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 pl-9 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent select-date" placeholder="Choose date..." type="text" onchange="this.form.submit()" readonly="readonly" value="<?= isset($_GET['date']) ? $_GET['date'] : date('Y-m-d')  ?>">
            ...
            <div style="margin-top: 1.5rem">
                <form action="/save-purchase-list" class="leadlist" method="post">
                    ...
                    <table class="datatable-init table stripe" style="font-size: 11px"> 
                        ...
                        <tbody>
                            <?php $no = 1; ?>
                            <?php foreach ($selections->getResultObject() as $data) : ?>                              
                                <tr class="border-y border-transparent border-b-slate-200 dark:border-b-navy-500">
                                    ...
                                    <td class="align-middle text-center"><?= round($data->roi,2) ?>%</td>                                                                                 
                                    <td class="text-center align-middle">
                                        <?php if (!empty($data->status)) : ?>
                                            <?php if (strtolower($data->status) == 'unrestricted') : ?>
                                                <div class="badge bg-success text-white shadow-soft shadow-success/50"  style="font-size: 11px;">
                                                    <?= strtoupper($data->status) ?>
                                                </div>
                                            <?php elseif (strtolower($data->status) == 'restricted') : ?>
                                    ...
                                    <!-- <td class="text-center align-middle">
                                        <input name="notes[]" class="form-input w-full rounded-lg border border-slate-300 bg-transparent px-3 py-1 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent" placeholder="Notes" type="text" data-id="<?= $data->id ?>" >                                            
                                    </td> -->
                                    <td class="text-center align-middle">
                    ...
                    <div class="text-center">
                        ...
                        <!-- <button type="submit" class="btn btn-danger"><em class="icon ni ni-save-fill me-1"></em>Save</button> -->
                    </div>
                </form>    
            </div> 
        </div>
                ...
                $(".filter-date").flatpickr({
                    disable: [
                        function (date) {
                            ...
                            var currDate = newDate.getFullYear()+ '-' + ('0' + (newDate.getMonth() + 1)).slice(-2) + '-' + ('0' + newDate.getDate()).slice(-2);
                            
                            
                            return (!availDates.includes(currDate));
                        }
                    ],                                            
                    altFormat: 'Y-m-d',
                    dateFormat: 'Y-m-d'
        ...
        $(document).on("click", ".buyerModal", function() {
            ...
            $.get("/get-purchase-item", {id : id})
                .done(function( data ) {
                    ...
                    $(".price_val").val(parseFloat(resp[0]['prmarket_priceice']).toFixed(2));                           
