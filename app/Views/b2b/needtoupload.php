...
<div class="grid grid-cols-1 gap-4 sm:gap-5 lg:gap-6">
    <div class="mb-1">        
        <form action="/b2b/need-to-upload/" method="GET" style="display: flex;">
        <?php csrf_field() ?>
            <div class="mr-4">
    ...
    <div class="card px-4 pb-2 sm:px-5">
        <div class="my-3 flex h-8 items-center justify-between" style="display: flex; ">
            ...
            <?php if (!is_null($start)) : ?>
                <?php if (!is_null($end)) : ?>
                    <a href="/b2b/export-need-to-upload/<?= $start ?>&<?= $end ?>"  class="btn border border-success/30 bg-success/10 font-medium text-success hover:bg-success/20 focus:bg-success/20 active:bg-success/25"><em class="fas fa-file-excel mr-1"></em> Export</a>             
                <?php else : ?>
                    <a href="/b2b/export-need-to-upload/<?= $start ?>"  class="btn border border-success/30 bg-success/10 font-medium text-success hover:bg-success/20 focus:bg-success/20 active:bg-success/25"><em class="fas fa-file-excel mr-1"></em> Export</a>             
                <?php endif ?>
            <?php else : ?>
                <a href="/b2b/export-need-to-upload"  class="btn border border-success/30 bg-success/10 font-medium text-success hover:bg-success/20 focus:bg-success/20 active:bg-success/25"><em class="fas fa-file-excel mr-1"></em> Export</a>             
            <?php endif ?>
...
<script>
    $(document).on('input propertychange', '.fnsku', function(data) {            
            ...
            $.post('/b2b/save-fnsku', {item: item, fnsku: fnsku})
                .done(function(data) {
        ...
        $(document).on('input propertychange', '.vendor', function(data) {            
            ...
            $.post('/b2b/save-vendor-name', {item: item, vendor: vendor})
                .done(function(data) {
        ...
        $(document).on('input propertychange', '.fba-number', function(data) {            
            ...
            inputTimer = setTimeout(function() {
                $.post('/b2b/save-fba-number', {box_name: boxName, number: number})
                    .done(function(data) {
        ...
        $(document).on('input propertychange', '.shipment-number', function(data) {            
            ...
            clearTimeout(inputTimer);
            inputTimer = setTimeout(function() {
                $.post('/b2b/save-shipment-number', {box_name: boxName, number: number})
                    .done(function(data) {
        ...
        $(document).on('input propertychange', '.dimensions', function(data) {            
            ...
            clearTimeout(inputTimer);
            inputTimer = setTimeout(function() {
                $.post('/b2b/save-dimensions', {box_name: boxName, dim: dim})
                    .done(function(data) {
