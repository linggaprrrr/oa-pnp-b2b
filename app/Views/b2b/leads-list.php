                                        ...
                                        <div class="flex justify-between items-center p-2 mx-3 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">                                                                            
                                            ...
                                            <div class="flex items-center" style="margin: 0">
                                                ...
                                                <span class="relative flex">
                                                    <select 
                                                        style="width: 160px; font-size: 12px; margin-left: 30px"
                                                        name="sheet-name[]" 
                                                        data-id="test"
...
<script type="module">
    ...
    $(document).on('change', '.sheet-list', function() {
        ...
        fileData.append('file', files[id]); 
        fileData.append('sheet-name', sheetName); 
        rowEntry = [];
        $.ajax({
            url: "/read-excel",
            type: "post",
        ...
        if (this.files.length > 20) {
            ...
            for (var i = 0; i < this.files.length; i++) {     
                ...
                readXlsxFile(this.files[i]).then(function(rows) {     
                    ...
                    // 'rows' contains the data read from the Excel file

                    // Process the data to remove formatting
                    const rawData = rows.map((row) => {
                        return row.map((cell) => {
                            // Check if the cell value is a string and remove formatting
                            return String(cell);
                        });
                    });
